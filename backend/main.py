...
@app.get("/personnages") 
def liste_personnages(db: Session = Depends(get_db)):
    return db.query(Personnage).all()

@app.post("/personnages")
def creer_peronnage(perso_data: model.Personnage, db: Session = Depends(get_db)):
...
def supprimer_personnage(id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.commit()
        return {"message": f"Personnage {id} supprimé avec succès"}
    except Exception as e:
        db.rollback()
...
def level_up(id: int, db: Session = Depends(get_db)):
#   niveau   += 1
#   pv       += 20
#   attaque  += 5
#   defense  += 3
